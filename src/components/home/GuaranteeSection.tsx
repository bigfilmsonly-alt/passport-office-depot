import { GUARANTEE } from '@/lib/constants';

export function GuaranteeSection() {
  return (
    <section className="px-5 pb-8">
      <div className="bg-green/5 border-2 border-green/20 rounded-2xl p-5 text-center">
        <div className="w-14 h-14 mx-auto mb-3 rounded-full bg-green/10 flex items-center justify-center">
          <svg className="w-7 h-7 text-green" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
          </svg>
        </div>
        <h3 className="font-display text-lg font-bold text-ink mb-1">
          100% On-Time Guarantee
        </h3>
        <p className="text-sm text-ink/70 leading-relaxed">
          {GUARANTEE}. We&apos;ve maintained a 99.8% success rate across 2M+ passports processed since 1976.
        </p>
      </div>
    </section>
  );
}
